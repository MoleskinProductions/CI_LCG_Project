"""Minimal CI Graph Overlay panel (PySide6/PySide2 + QGraphicsScene)."""
...
def _qt_horizontal():
    try:
        return QtCore.Qt.Orientation.Horizontal  # Qt6 style
    except Exception:
        return QtCore.Qt.Horizontal  # Qt5 style


def _qt_antialiasing():
    try:
        return QtGui.QPainter.RenderHint.Antialiasing  # Qt6 style
    except Exception:
        return QtGui.QPainter.Antialiasing  # Qt5 style
...
class CIGraphOverlayPanel(QtWidgets.QWidget if QtWidgets else object):
    ...
    def __init__(self, parent=None):
        ...
        # File lives under houdini_package/python/ci_panels/
        # so parents[1] is already houdini_package/python.
        pkg_python = Path(__file__).resolve().parents[1]
    ...
    def _build_ui(self):
        ...
        body.setOrientation(_qt_horizontal())
        ...
        self.view.setRenderHint(_qt_antialiasing(), True)
    ...
    def _suggestions_jsonl_path(self) -> Path:
        self._paths_mod.ensure_runtime_dirs()
        return self._paths_mod.task_suggestions_path()
